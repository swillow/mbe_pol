...
arma::mat compute_shellblock_norm(const BasisSet &obs, const arma::mat &A);

arma::vec compute_2body_ints(const BasisSet &bs, const arma::mat &Km);

Integrals::Integrals(const vector<Atom> &atoms, const BasisSet &bs,
                     const vector<QAtom> &Q_atoms) {

  if (atoms.size() == 0)
    return;

  Sm = compute_1body_ints<Operator::overlap>(bs, atoms, Q_atoms);
  Tm = compute_1body_ints<Operator::kinetic>(bs, atoms, Q_atoms);
...
template <Operator obtype>
arma::mat Integrals::compute_1body_ints(const BasisSet &bs,
                                        const vector<Atom> &atoms,
                                        const vector<QAtom> &Q_atoms) {
  const auto nbf = bs.nbf();
  const auto nshells = bs.size();

  arma::mat result(nbf, nbf, arma::fill::zeros);
  ...
  if (obtype == Operator::nuclear) {
    ...
    if (Q_atoms.size() > 0) {
      ...
      for (const auto &q_atom : Q_atoms) {
        q.push_back({q_atom.charge, {{q_atom.x, q_atom.y, q_atom.z}}});
      }
    }
...
arma::mat compute_schwartz_ints(const BasisSet &bs) {
  ...
  arma::mat result(nsh, nsh, arma::fill::zeros);

  // Construct the 2-electron repulsion integrals engine
  libint2::Engine engine =
      libint2::Engine(Operator::coulomb, bs.max_nprim(), bs.max_l(), 0);
  ...
  for (auto s1 = 0, s12 = 0; s1 != nsh; ++s1) {
    ...
    for (auto s2 = 0; s2 <= s1; ++s2, ++s12) {
      ...
      engine.compute2<Operator::coulomb, BraKet::xx_xx, 0>(bs[s1], bs[s2],
                                                           bs[s1], bs[s2]);
      const auto *buf0 = buf[0];
...
arma::vec compute_2body_ints(const BasisSet &bs, const arma::mat &Schwartz) {
...
arma::mat compute_2body_fock(const BasisSet &bs,
  ...
  // Construct the 2-electron repulsion integrals engine
  libint2::Engine engine(Operator::coulomb, bs.max_nprim(), bs.max_l(), 0);
  ...
  for (auto s1 = 0; s1 != nshells; ++s1) {
    ...
    for (auto s2 = 0; s2 <= s1; ++s2) {
      ...
      for (auto s3 = 0; s3 <= s1; ++s3) {
        ...
        const auto Dnorm123 =
            max(D_shblk_norm(s1, s3), max(D_shblk_norm(s2, s3), Dnorm12));
        ...
        for (auto s4 = 0; s4 <= s4_max; ++s4) {
          ...
          const auto Dnorm1234 = max(
              D_shblk_norm(s1, s4),
              max(D_shblk_norm(s2, s4), max(D_shblk_norm(s3, s4), Dnorm123)));
...
arma::mat compute_shellblock_norm(const BasisSet &bs, const arma::mat &A) {
  const auto nsh = bs.size();
  ...
  for (size_t s1 = 0; s1 != nsh; ++s1) {
    ...
    for (size_t s2 = 0; s2 != nsh; ++s2) {
      ...
      auto Ablk = A.submat(s1_first, s2_first, s1_last, s2_last);
